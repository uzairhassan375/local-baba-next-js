...
export default function AdminProductsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (imageUrls.length) {
        // Any pasted URL not already on our Bunny CDN gets fetched and re-hosted there.
        imageUrls = await rehostImageUrlsToBunny(imageUrls, slug);
      }
      if (pendingFiles.length) {
        const uploaded = await uploadProductImages(pendingFiles, slug);
        if (
